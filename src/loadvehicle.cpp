...
bool LoadSuspension(
	...
	std::ostream & error)
{
	if (!cfg_wheel.get("position", info.hub, error)) return false;
	if (!cfg_wheel.get("camber", info.camber, error)) return false;
	if (!cfg_wheel.get("caster", info.caster, error)) return false;
	if (!cfg_wheel.get("toe", info.toe, error)) return false;
