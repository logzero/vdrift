	...
	std::ostream & error_output)
{
	...
	float timerbox_lowery = 0;
	{
		...
		if (!content.load(timerboxtex, texturepath, "timerbox.png", timerboxtexinfo)) return false;
	...
	if (!content.load(bartex, texturepath, "hudbox.png", texinfo)) return false;
	if (!content.load(progbartex, texturepath, "progressbar.png", texinfo)) return false;
