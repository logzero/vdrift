	...
	std::ostream & error_output)
{
	...
	if (!content.load(track_map, "", trackname, texinfo))
	{
	...
	TEXTUREINFO dotinfo;
	if (!content.load(cardot0, texturepath, "cardot0.png", dotinfo))
	{
		error_output << "Can't load cardot0" << std::endl;
		return false;
	}
	if (!content.load(cardot1, texturepath, "cardot1.png", dotinfo))
	{
		error_output << "Can't load cardot1" << std::endl;
		return false;
	}
	if (!content.load(cardot0_focused, texturepath, "cardot0_focused.png", dotinfo))
	{
		error_output << "Can't load cardot0_focused" << std::endl;
		return false;
	}
	if (!content.load(cardot1_focused, texturepath, "cardot1_focused.png", dotinfo))
	{
