...
void SOUNDDEVICE_CallbackWrapper(void *soundclass, Uint8 *stream, int len)
{
	((SOUNDDEVICE*)soundclass)->Callback16bitstereo(soundclass, stream, len);
}

bool SOUNDDEVICE::Init(int buffersize, std::ostream & info_output, std::ostream & error_output)
{
	if (disable || initdone)
		return false;
	...
	desired.callback = SOUNDDEVICE_CallbackWrapper;
...
void SOUNDDEVICE::Pause(const bool pause_on)
{
...
void SOUNDDEVICE::Callback16bitstereo(void *myself, Uint8 *stream, int len)
{
	assert(this == myself);
	assert(initdone);
...
void SOUNDDEVICE::CollectGarbage()
{
	if (disable)
		return;
...
void SOUNDDEVICE::DetermineActiveSources(std::list <SOUNDSOURCE *> & active_sourcelist, std::list <SOUNDSOURCE *> & inaudible_sourcelist) const
{
...
void SOUNDDEVICE::RemoveSource(SOUNDSOURCE * todel)
{
	if (disable)
		return;
...
void SOUNDDEVICE::Compute3DEffects(std::list <SOUNDSOURCE *> & sources, const MATHVECTOR <float, 3> & listener_pos, const QUATERNION <float> & listener_rot) const
{
...
void SOUNDDEVICE::LockSourceList()
{
...
void SOUNDDEVICE::UnlockSourceList()
{
